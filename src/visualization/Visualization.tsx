...
export function Visualization({ onAnnotationClick, onAnnotationDelete, onNegativeAccountWarning, onChartClick, onEditEnvelope }: VisualizationProps) {
  ...
  const [allNetWorthData, setAllNetWorthData] = useState<Datum[]>([]); // contains everyday networth data and parts
  ...
  const sampleData = useCallback(async (
    ...
    setNetWorthData(sampledNetWorthData);
    //Sample the locked networth data at the time points and save to locked networth data
    const sampledLockedNetWorthData = allLockedNetWorthData.filter((data: Datum) => timePoints.includes(data.date));
    setLockedNetWorthData(sampledLockedNetWorthData);

  }, [allNetWorthData, allLockedNetWorthData, timeInterval, currentDay]);
        ...
        {(zoom) => {
          ...
          useEffect(() => {
            ...
            if (newInterval !== timeInterval) {
              ...
              sampleData(newInterval, paddedRange);
